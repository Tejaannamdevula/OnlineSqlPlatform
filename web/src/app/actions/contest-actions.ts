...
// Check if user has access to a contest
export async function checkContestAccess(contestId: string) {
	try {
		const session = await verifySession();
		...
		// If contest is not protected, allow access
		if (!contest.protectedContest) {
			return { success: true };
		}

		// Check if user has already been granted access (via cookie)
		...
		// If contest is protected and no access cookie, require password
		return {
			success: false,
			requiresPassword: true,
			error: "Password required for this contest",
...
export async function getContestLeaderboard(contestId: string) {
	try {
		...
		// Get leaderboard data with rank calculation - fixed SQL query
		const leaderboardData = await appDb.execute(sql`
      WITH RankedPoints AS (
        SELECT 
          ${contestPoints.userId} as userId,
          ${contestPoints.pointsEarned} as pointsEarned,
          RANK() OVER (ORDER BY ${contestPoints.pointsEarned} DESC) as rank
        FROM ${contestPoints}
        WHERE ${contestPoints.contestId} = ${contestId}
      )
